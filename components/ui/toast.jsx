...
const Toast = React.forwardRef(({ className, variant, ...props }, ref) => {
  const getIcon = (variant) => {
    switch (variant) {
      case "success":
        return <AiOutlineCheckCircle className="h-5 w-5 text-green-500" />;
      case "destructive":
        return <AiOutlineCloseCircle className="h-5 w-5 text-red-500" />;
      case "info":
        return <AiOutlineInfoCircle className="h-5 w-5 text-yellow-500" />;
      default:
        return null;
    }
  };
...
const ToastAction = React.forwardRef(({ className, ...props }, ref) => (
  <ToastPrimitives.Action
    ref={ref}
    className={cn(
      "inline-flex h-8 shrink-0 items-center justify-center rounded-md border bg-transparent px-3 text-sm font-medium transition-colors duration-200 ease-in-out hover:bg-secondary hover:shadow-md focus:outline-none focus:ring-2 focus:ring-ring disabled:pointer-events-none disabled:opacity-50 group-[.destructive]:border-muted/40 group-[.destructive]:hover:border-destructive/30 group-[.destructive]:hover:bg-destructive group-[.destructive]:hover:text-destructive-foreground group-[.destructive]:focus:ring-destructive",
      className
    )}
    {...props}
  />
